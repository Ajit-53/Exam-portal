<?php
	...
	if(isset($_POST['delete'])){
		$query = "DELETE FROM questions WHERE ques = '".$_POST['dq']."'";
		mysqli_query($con, $query);
		$query = "ALTER TABLE answers DROP COLUMN a".$n;
		mysqli_query($con, $query);
	}
?>
	...
	<body>
		...
		<div class="container">
			...
			<div class="col-lg-12 bg-light shadow-lg" id="d1" style="margin: 10px; padding: 10px;">
				...
				<a href="addQues.php"><button class="btn btn-primary">Add new question</button></a>
					<form action="admin.php" method="post" onsubmit="return getQues(<?php echo $n; ?>)">
						Enter Question Number: <input type="text" name="dq" id="delete_ques" width="100%">
		...
		<script>
			...
			function getQues(n){
				var q = document.getElementById("delete_ques");
				var num = q.value;
				if(isNaN(num) || parseInt(num) > n){
					return false;
				}
				else{
					var ques = document.getElementsByName("t"+num)[0].value;
					q.value = ques;
					return true;
				}
			}
